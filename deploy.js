...
function createRemoteTempFolder() {
  return ssh.execCommand(
    'rm -rf deploy-final-temp && mkdir deploy-final-temp', {
      cwd: '/home/ubuntu'
  });
}
...
function updateRemoteApp() {
  return ssh.execCommand(
    'cp -r deploy-final-temp/* deploy-final/ && rm -rf deploy-final-temp', {
      cwd: '/home/ubuntu'
  });
}
...
function restartRemoteServices() {
  return ssh.execCommand(
    'cd deploy-final && sudo service mongod start && pm2 start app.js', {
      cwd: '/home/ubuntu'
  });
}
...
function sshConnect() {
  ...
  ssh
    ...
    .then(function() {
      console.log('Creating `deploy-final-temp` folder.');
